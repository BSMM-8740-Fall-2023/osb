...
Create workflows with `workflowsets::workflow_set` using your recipe and models. Show the workflow datastructure, noting the number of columns, and then use `tidy::` to unnest the *info* column and show its contents.
...
Use workflowsets::workflow_map to map the default function (`tune::tune_grid()` - look at the help) across the workflows in the workflowset you just created and update the variable `all_workflows` with the result.
...
The are 0.06821716/0.05747611 - 1 = 0.1868785, or about 19% higher in the test dataset than in the training dataset.What is the ration of the OOB prediction errors (MSE): test/train?
:::
