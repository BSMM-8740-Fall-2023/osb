...
-   Today we will explore time series - data where each observation includes a time value.
...
A time series data is a data frame (tibble) with an ordered temporal variable.
...
\text{(noise)}\qquad I_{t} & =0.5I_{t-1}+\epsilon_{t}\quad\epsilon_t\sim\mathcal{N}(0,\sigma^2)
\end{align*}
$$
...
dat %>% 
  ...
  ggplot(aes(x=date, y=y)) + geom_line() +
  labs(
...
$$\epsilon_{t}\sim\text{{i.i.d.}}\mathcal{N}\left(0,\sigma_{\epsilon_{t}}^{2}\right)$$
:::
