...
One of the important assumptions of the classical linear regression models is that there is no exact collinearity among the regressors. While high correlation between regressors is a necessary indicator of the problem, a direct linear relationship beween regressors is sufficient.
...
Can we re-write the glm coefficient plot code with only tidyverse verbs? ::: panel-tabset \## as written
...
So here the link function is $\log$ and $\log\mathbb{E}\left[\left.Y\right|X\right] = \beta X$
...
plot(data_test$medv, pr.nn$net.result, col = "red", 
