...
We can fit a linear regression with OLS (model spec via formula) or penalized regression (x,y spec, i.e. independent data components) using data structured specifically for the model.
...
```{r}
#| echo: true
parsnip::linear_reg(penalty = 1) %>% parsnip::set_engine("glmnet") %>% 
  parsnip::translate()
```
:::
...
We can define the model with either a formula or outcome and model matrix:
...
If we use the term **model** to reference a structural equation that relates some predictors to one or more outcomes, then everything before the model is fit is related to structuring the predictors and outcomes.
